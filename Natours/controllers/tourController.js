...
exports.uploadeTourImages = upload.fields([{ name: 'imageCover', maxCount: 1 }, { name: 'images', maxCount: 3 }])

exports.resizeTourImages = catchAsync(async (req, res, next) => {
	if (!req.files.imageCover || !req.files.images) return next()

	//* 1) Cover image
	// UpdateOne in handlerFactory.js parses the hole body, so we have to add the filename to the body
	...
	//* 2) Images */
	// UpdateOne in handlerFactory.js parses the hole body, so we have to add all the filesnames to the body
	req.body.images = []
	await Promise.all(
